...
static void atk_mc5640_get_pre_scaling_size(void) {
  ...
  g_atk_mc5640_sta.pre_scaling.width =
      g_atk_mc5640_sta.isp_input.width - (x_offset << 1);
  g_atk_mc5640_sta.pre_scaling.height =
      g_atk_mc5640_sta.isp_input.height - (y_offset << 1);
}
...
uint8_t atk_mc5640_init(void) {
  ...
  SCCB_Init();                  /* ATK-MC5640 SCCB接口初始化 */
...
uint8_t atk_mc5640_auto_focus_init(void) {
  ...
  for (addr_index = ATK_MC5640_FW_DOWNLOAD_ADDR, fw_index = 0;
       fw_index < sizeof(atk_mc5640_auto_focus_firmware);
       addr_index++, fw_index++) {
 ...
 */
uint8_t
atk_mc5640_set_color_saturation(atk_mc5640_color_saturation_t saturation) {
  switch (saturation) {
...
uint8_t atk_mc5640_set_output_format(atk_mc5640_output_format_t format) {
  ...
  case ATK_MC5640_OUTPUT_FORMAT_JPEG: {
    for (cfg_index = 0; cfg_index < sizeof(atk_mc5640_jpeg_cfg) /
                                        sizeof(atk_mc5640_jpeg_cfg[0]);
         cfg_index++) {
      atk_mc5640_write_reg(atk_mc5640_jpeg_cfg[cfg_index].reg,
                           atk_mc5640_jpeg_cfg[cfg_index].dat);
    }
    break;
  }
  case ATK_MC5640_OUTPUT_FORMAT_YUV422: {
    for (cfg_index = 0; cfg_index < sizeof(atk_mc5640_yuv422_cfg) /
                                        sizeof(atk_mc5640_yuv422_cfg[0]);
         cfg_index++) {
      atk_mc5640_write_reg(atk_mc5640_yuv422_cfg[cfg_index].reg,
                           atk_mc5640_yuv422_cfg[cfg_index].dat);
    }
    break;
  }
  default: {
 ...
 *              ATK_MC5640_EINVAL: 传入参数错误
 */
uint8_t atk_mc5640_set_isp_input_window(uint16_t x, uint16_t y, uint16_t width,
                                        uint16_t height) {
  uint8_t reg3800;
  uint8_t reg3801;
  uint8_t reg3802;
  ...
  if ((x_end > ATK_MC5640_ISP_INPUT_WIDTH_MAX) ||
      (y_end > ATK_MC5640_ISP_INPUT_HEIGHT_MAX)) {
    return ATK_MC5640_EINVAL;
  }
 ...
 */
uint8_t atk_mc5640_set_pre_scaling_window(uint16_t x_offset,
                                          uint16_t y_offset) {
  uint8_t reg3810;
  uint8_t reg3811;
  uint8_t reg3812;
 ...
 * 图像数据以字方式写入目的地址，目的地址自动增加 before_transfer:
 * 帧数据传输前，需要完成的事务，可为NULL
 * @retval      ATK_MC5640_EOK   : 获取ATK-MC5640模块输出的一帧图像数据成功
 *              ATK_MC5640_EINVAL: 传入参数错误
 *              ATK_MC5640_EEMPTY: 图像数据为空
 */
uint8_t atk_mc5640_get_frame(uint32_t dts_addr, atk_mc5640_get_type_t type,
  ...
  /* 等待传输完成 (阻塞式，或者在应用层通过查询 g_ov5640_frame_cplt
   * 来实现非阻塞) */
...
/**
 * @brief       HAL DCMI Frame Event Callback
