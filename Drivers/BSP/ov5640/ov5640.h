...
typedef enum {
  ...
  ATK_MC5640_OUTPUT_FORMAT_YUV422,        /* YUV422 */
} atk_mc5640_output_format_t;

/* ATK-MC5640获取帧数据方式枚举 */
typedef enum {
  ATK_MC5640_GET_TYPE_DTS_8B_NOINC =
      0x00, /* 图像数据以字节方式写入目的地址，目的地址固定不变 */
...
uint8_t
atk_mc5640_auto_focus_continuance(void); /* ATK-MC5640模块持续自动对焦 */
void atk_mc5640_led_on(void);            /* 开启ATK-MC5640模块闪光灯 */
void atk_mc5640_led_off(void);           /* 关闭ATK-MC5640模块闪光灯 */
uint8_t atk_mc5640_set_light_mode(
    atk_mc5640_light_mode_t mode); /* 设置ATK-MC5640模块灯光模式 */
uint8_t atk_mc5640_set_color_saturation(
    atk_mc5640_color_saturation_t saturation); /* 设置ATK-MC5640模块色彩饱和度
                                                */
uint8_t atk_mc5640_set_brightness(
    atk_mc5640_brightness_t brightness); /* 设置ATK-MC5640模块亮度 */
...
uint8_t atk_mc5640_set_mirror_flip(
    atk_mc5640_mirror_flip_t mirror_flip); /* 设置ATK-MC5640模块镜像/翻转 */
uint8_t atk_mc5640_set_test_pattern(
    atk_mc5640_test_pattern_t pattern); /* 设置ATK-MC5640模块测试图案 */
uint8_t atk_mc5640_set_output_format(
    atk_mc5640_output_format_t format); /* 设置ATK-MC5640模块输出图像格式 */
uint8_t atk_mc5640_set_isp_input_window(
    uint16_t x, uint16_t y, uint16_t width,
    uint16_t height); /* 设置ATK-MC5640模块ISP输入窗口尺寸 */
uint8_t atk_mc5640_set_pre_scaling_window(
    uint16_t x_offset,
    uint16_t y_offset); /* 设置ATK-MC5640模块预缩放窗口偏移 */
uint8_t atk_mc5640_set_output_size(
    uint16_t width, uint16_t height); /* 设置ATK-MC5640模块输出图像尺寸 */
uint8_t atk_mc5640_get_frame(
    uint32_t dts_addr, atk_mc5640_get_type_t type,
    void (*before_transfer)(void)); /* 获取ATK-MC5640模块输出的一帧图像数据 */
